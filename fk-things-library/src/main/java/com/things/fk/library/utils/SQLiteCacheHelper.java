...
public abstract class SQLiteCacheHelper {
    ...
    public boolean updateWithinTransaction(List<ContentValues> values, String whereClause, List<String> whereArgs) {
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        try {
            db.beginTransaction();
            for (int i = 0; i < values.size(); i++) {
                db.updateWithOnConflict(mTableName, values.get(i), whereClause, new String[]{whereArgs.get(i)},
    ...
    public void delete(String whereClause, String[] whereArgs) {
        ...
        try {
            Log.e(TAG, "delete:" + whereClause + " args:" + Arrays.toString(whereArgs));
    ...
    public void insertOrReplace(ContentValues values) {
        ...
        try {
            long row = mOpenHelper.getWritableDatabase().insertWithOnConflict(
                    mTableName, null, values, SQLiteDatabase.CONFLICT_REPLACE);
            Log.e(TAG, "insert or replace " + row);
        } catch (SQLiteFullException e) {
            onDiskFull(e);
        } catch (SQLiteException e) {
            Log.d(TAG, "Ignoring sqlite exception", e);
